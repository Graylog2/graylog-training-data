...
#Get Content Pack
wget https://github.com/Graylog2/graylog-training-data/raw/main/instruqt/context_enrichment/scripts/setup_scripts/setup-enrichment_context-enrich_challenge.json

id=$(cat setup-enrichment_context-enrich_challenge.json | jq -r '.id')
ver=$(cat setup-enrichment_context-enrich_challenge.json | jq -r '.rev')
curl -u 'admin:yabba dabba doo' -k -XPOST "https://localhost/api/system/content_packs"  -H 'Content-Type: application/json' -H 'X-Requested-By: PS_Packer' -d @"setup-enrichment_context-enrich_challenge.json" 
